...
function SaveJsonFiles(IndexFilePath) {
  ...
  let files = getFiles.getFileList('./src/json/');
  files.forEach((item) => {
...
module.exports = (env) => {
  ...
  //获取文件夹下的所有文件
  let files = getFiles.getFileList('./src/html/');
  let DivHtml = '';
  ...
  return {
    entry: {
      app: [
        ...
        'jquery',
        'babel-polyfill',
        path.resolve(__dirname, 'src/' + IndexFilePath + '/' + IndexFilePath + '.js'),
    ...
    plugins: [
      ...
      new HtmlWebpackPlugin({
        filename: '../../src/html/' + IndexFilePath + '.html',
